...
class CustomCommand extends Command {
      ...
      args: [
        {
          ...
          default: "",
          match:
            "/^(yes|no|true|false|d+|on|off|null|undefined|nan|toggle|switch|flip|!+)$/",
        },
      ],
  ...
  async exec(message, bool) {
    ...
    if (!bool.length) {
      //query current value
      return message.send(
        "Current value is: " + this.client.memory.channelGet(message, varName)
      );
    }

    var toggle = bool.match(/^(toggle|switch|flip|!+)$/);
    if (toggle) {
      var value = this.client.memory.channelSet(
        message,
        varName,
        !!this.client.memory.channelGet(message, varName)
      );
      return message.send("Toggled value to: " + value);
    }

    var value = bool.match(/^(no|false|0|off|null|undefined|nan)$/)
      ? false
      : true;
