...
class CustomListener extends Listener {
  ...
  async exec(reaction, user) {
    ...
    let sendToUser =
      /*message.guild.member(message.member.user) ||*/ message.member;
    ...
    console.log(
      `${name} reacted with "${reaction.emoji.name}" to ${sendToUser.displayName}'s ${message.id} with content ${messagePreview}.`
    );

    /** Emoji alerts **/
    let key = `${message.id}/${userID}`;
    let cache = this.client.memory.channelGet(message, "reactionCache", {});
    let cacheFunctions = this.client.memory.channelGet(
      message,
      "reactionListener",
      {}
    );
    ...
    if (!cacheFunction) {
      cacheFunction = cacheFunctions[key] = _.debounce(function () {
        ...
        embed.setAuthor(
          `${name} reacted ${reactions}`,
          (member.user
            ? member.user.displayAvatarURL()
            : member.displayAvatarURL()) || common.defaultAvatar,
          `https://discordapp.com/users/${member.id}`
        );
        let permalink = util.messages.permalink(message);
        embed
          .setDescription(
            `channel: [${message.channel.name}](${permalink})\nmessage: [${messagePreview}](${permalink})`
          )
          .setFooter(`ID: ${message.id}`)
          .setTimestamp();

        let logChannel = message.guild.channels.resolve(
          config.actionLogChannel
        );
        logChannel && logChannel.send(embed);

        //see if user wants notificaiton
        config.debug && console.log(sendToUser.roles.cache);
        let notify = sendToUser.roles.cache.find(
          (r) => r.name === "ReceiveReactAlert"
        );
