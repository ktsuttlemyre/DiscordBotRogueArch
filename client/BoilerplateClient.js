...
class BoilerplateClient extends AkairoClient {
  constructor(opts) {
    ...
    this.commandHandler
      .on("commandBlocked", function (message, command, reason) {
      ...
      .on("error", function (error, message, command) {
        ...
        if (command) {
          message.channel.send(
            `\`${commandName}\` got error ${error.name}: ${error.message}`
          );
        } else {
      ...
      .on("inPrompt", function (message) {
        ...
        message.channel.send(
          `Currently talking to ${name} please wait for them to finish.`
        );
      })
      //.on('load',function(){})
      ...
      .on("missingPermissions", function (message, command, type, missing) {
        ...
        let isRole = message.guild.roles.cache.find(
          (role) => missing == role.name.toLowerCase()
        );
        if (isRole) {
          return message.channel.send(
            `${type} must have permission ${missing} role in order to execute \`${commandName}\``
          );
        }
        message.channel.send(
          `${type} must have permission ${missing} in order to execute \`${commandName}\``
        );
      });
