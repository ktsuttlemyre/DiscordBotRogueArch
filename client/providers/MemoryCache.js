...
class MemoryCache {
  ...
  channelGet(message, key, defaultValue) {
    ...
    if (!message.channel) {
      throw new Error(
        "Memory ChannelGet needs to have a message with a proper channel object passed to it"
      );
    }
    const channelID = this.constructor.getChannelID(message.channel);
    if (!channelID) {
      throw new Error(
        "Channel.id must be specified if you want to get a channel value in memory"
      );
    }
    id = `${id}/${channelID}`;
  ...
  channelSet(message, key, value) {
    ...
    if (!message.channel) {
      throw new Error(
        "Memory ChannelSet needs to have a message with a proper channel object passed to it"
      );
    }
    const channelID = this.constructor.getChannelID(message.channel);
    if (!channelID) {
      throw new Error(
        "Channel.id must be specified if you want to set a channel value in memory"
      );
    }
    id = `${id}/${channelID}`;
    if (!cache[id]) {
  ...
  static getChannelID(channel) {
    ...
    if (typeof id === "string" && /^\d+$/.test(id)) return id;
    throw new TypeError(
      "Invalid channel specified. Must be a Channel instance"
    ); //, guild ID, "global", or null.');
  }
}
