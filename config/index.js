...
let loadToneMap = async function (configInstance) {
  ...
  configInstance.voiceJoinLeave.tones.custom = Object.assign(
    configInstance.voiceJoinLeave.tones.custom,
    json
  );
};
...
if (
  configInstance.voiceJoinLeave &&
  configInstance.voiceJoinLeave.tones &&
  configInstance.voiceJoinLeave.tones.externalMap
) {
  let fn = function () {
    loadToneMap(configInstance);
  };
  setInterval(
    fn,
    configInstance.voiceJoinLeave.tones.externalMapUpdateInterval ||
      15 * 60 * 1000
  ); //15 minutes
  fn();
}
