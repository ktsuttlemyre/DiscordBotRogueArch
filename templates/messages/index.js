...
const presentTitle = (exports.presentTitle = function (string) {
  ...
  var after = string.replace(
    /[\[\(]\W*(official)?\W*(music)?\W*(video|audio)\W*(version)?\W*[\)\]]/i,
    ""
  );
...
var nowPlayingOverloaded = async (message, player, announce) => {
  ...
  if (typeof announce == "object") {
    ...
    announceObj.description =
      announceObj.description.length >= 2048
        ? `${announceObj.description.substr(0, 2045)}...`
        : undefined;
  }
  ...
  let progressBar = (
    player.createProgressBar(message, { queue: false, timecodes: true }) || ""
  )
    .replace("▬", "")
    .replace("🔘", stateButton)
    .replace("┃ ", "|")
  ...
  let trackDiscordMessage = await util.messages.retrieveTrackMessage(
    message,
    track
  );
  ...
  if (!player.backgroundPlaylist && track.requestedBy) {
    ...
    let name = encodeURIComponent(
      track.requestedBy.displayName || track.requestedBy.tag
    );
    let URI = encodeURI(`https://shiptunes.shipwa.sh/${track.requestedBy.id}`);

    if (name) {
      URI = encodeURI(
        `https://shiptunes.shipwa.sh/${track.requestedBy.id}#?name=${name}`
      );
    }
    embedJSON.author = {
...
var nowPlayingSegments = function (message, track, player) {
  ...
  var stateButton = track.queue.stopped
    ? ":stop_button:"
    : track.queue.paused
    ? ":pause_button:"
    : ":arrow_forward:";
  ...
  if (track.queue.tracks[1]) {
    var nextPlaying = {
      ...
      footer: {
        text:
          track.queue.tracks[1].requestedBy.username +
          " requested current song",
  ...
  var playerState = {
    ...
    fields: [
      ...
      },
      {
        name: "‎",
        value:
          (!track.queue.loopMode ? ":bell:" : ":bell:") +
          "Attention " +
          volumeLevel +
          " " +
          (track.queue.volume >= 100 ? ":100:" : track.queue.volume),
        inline: true,
      },
      {
        name: `Queue:`,
        value:
          stateButton +
          player.createProgressBar(message, { queue: true, timecodes: false }),
        inline: false,
      },
      {
        name: "‎",
