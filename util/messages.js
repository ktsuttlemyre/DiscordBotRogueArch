...
module.exports.retrieveTrackMessage = function (message, track) {
  var id = message.client.memory.channelGet(
    message,
    web.getYoutubeHash(track.url) + "_" + track.requestedBy.id + "_" + message
  ); // || this.client.memory.channelSet(message, 'player', util.player.create(message,this.client));
...
module.exports.encapsulate = async function (message, override, dontDelete) {
  ...
  let isAdmin =
    ...
    user.roles.cache.find((role) =>
      config.encapsulateAdminRoles.includes(role.name)
    );
...
module.exports.resolve = async function (message) {
  ...
  if (message.partial) {
    try {
      ...
      console.error(
        `Something went wrong when fetching the message: ${message.id}`,
        error
      );
      return message;
    }
...
module.exports.fetchMessagesLegacy = async function fetchMessages(
  channel,
  options,
  callback
) {
...
module.exports.fetchForEach = async function fetchMessages(
  channel,
  options,
  callback
) {
  ...
  let _processTick = async function (resolve) {
    ...
    for (
      let index = gIndex + gOffset, l = array.length;
      (loadedAllMessages || index < l - nBuffer) && index < l;
      index++, gIndex++
    ) {
...
module.exports.findInMessage = function findInMessage(message, target, opts) {
  ...
  if (
    (opts.caseSensitive && message.content.includes(str)) ||
    (!opts.caseSensitive && message.content.toLowerCase().includes(str))
  )
    return true;
    ...
    if (
      ...
      (!opts.caseSensitive &&
        ((opts.author && (embed.author || "").toLowerCase().includes(str)) ||
          (opts.description &&
            (embed.description || "").toLowerCase().includes(str)) ||
