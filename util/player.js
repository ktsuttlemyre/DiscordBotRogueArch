...
module.exports.commandPermissions = function (message, requireDJ) {
  ...
  let isDJ = message.member.roles.cache.find(
    (role) => role.name === config.DJ_Role
  );
  //DJ bypass
  if (isDJ) {
    return null;
  ...
  if (
    ...
    return `${emotes.error} - You are not in the same voice channel !`;
  //if the user is the only one in the channel then allow action
  if (
    channel &&
    channel.members.filter((member) => !member.user.bot).size == 1
  ) {
    return null;
  }
  //do voting (optional)
...
let playBackgroundPlaylist = (module.exports.playBackgroundPlaylist = async (
  message,
  player,
  notice
) => {
  ...
  let fetchNext =
    message.client.memory.channelGet(message, "backgroundPlaylistFetcher") ||
    defaultFetcher;
  ...
  player
    .on("trackStart", function (message, track) {
      if (track.skip) {
        ...
        GUIMessages.nowPlaying(
          message,
          player,
          "Skipping ${track.name} for reason:${track.skip}"
        );
      }
    ...
    .on("trackAdd", async (message, queue, track) => {
      ...
      let resolve = message.client.memory.channelGet(
        message,
        `${message.id}_resolve`
      );
      if (!resolve) {
      ...
      var embed = {
        ...
        callback: async function (reply) {
          ...
          await message.client.memory.channelSet(
            message,
            util.getYoutubeHash(track.url) +
              "_" +
          ...
          reply.permalink = common.permalinkMessage(
            reply.guild,
            reply.channel,
            reply
          );
          ...
          const collector = reply.createReactionCollector((reaction, user) => {
            return [reactions.upvote, reactions.downvote].includes(
              reaction.emoji.name
            );
          }); //{ time: 15000 }
          ...
          collector.on("collect", async (reaction, user) => {
            if (reaction.emoji.name === reactions.downvote) {
              ...
              let ogPosterID = (originalPoster.url || "")
                .split("#")
                .shift()
                .split("/")
                .pop();
              if (user.id === ogPosterID) {
                ...
                GUIMessages.nowPlaying(
                  message,
                  player,
                  `${user.username} removed ${track.title}`
                );
              } else {
                ...
                GUIMessages.nowPlaying(
                  message,
                  player,
                  `${user.username} does not like ${track.title}`
                );
              }
              ...
              GUIMessages.nowPlaying(
                message,
                player,
                `${user.username} likes ${track.title}`
              );
            }
          ...
          GUIMessages.nowPlaying(
            message,
            player,
            `${message.member.displayName} has added ${track.title}`
          );
        },
      };
    ...
    .on("playlistAdd", function (message, queue, playlist) {
      ...
      GUIMessages.nowPlaying(
        message,
        player,
        `${message.member.displayName} has added playlist ${playlist.title}`
      );
    })
    ...
    .on("queueEnd", async function (message, queue) {
      ...
      playBackgroundPlaylist(
        message,
        player,
        "Playing background music until I get a new request"
      );
    })
    .on("channelEmpty", function (message, queue) {
      GUIMessages.nowPlaying(
        message,
        player,
        "I am alone in the voice channel. :frowning:"
      );
    ...
    .on("botDisconnect", function (message) {
      GUIMessages.nowPlaying(
        message,
        player,
        "Music stopped. I have been disconnected from the channel!"
      );
    })
