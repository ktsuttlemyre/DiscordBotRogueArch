...
module.exports.fetchShift = function (subreddit, options) {
  ...
  return function () {
    return new Promise(async (resolve) => {
      ...
      if (!loadedAllItems && index > items.length - nBuffer) {
        //if we have items on the server and getting close to buffer then
        itemsResponse = await subredditBatch(subreddit, opts);
        if (
          !itemsResponse ||
          !itemsResponse.data ||
          !itemsResponse.data.children ||
          !itemsResponse.data.children.length
        ) {
          loadedAllItems = true;
          responseLength = 0;
        } else {
      ...
      if (loadedAllItems && index > items.length) {
...
let subredditBatch = (module.exports.subredditBatch = function (
  subreddit,
  opts
) {
  opts = opts || {};
