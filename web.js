...
web.toString = function (obj, override) {
	...
	//handle xml
	if (web.instanceOf(obj, window.Document) && !web.instanceOf(obj, window.HTMLDocument) && !web.instanceOf(obj, window.SVGDocument)) {
...
web.onlyOne = function (target, silentForce) {
	return target.length == 1 || silentForce ? target[0] : console.error("Expected to get only one ouput for the array", target);
};
web.top = function () {};
...
if (!Object.keys) {
	Object.keys = function (obj) {
		if (!force && o !== Object(o)) throw new TypeError("Object.keys called on a non-object");
