...
web.isInstance = web.instanceOf = function (obj, equals, deep) {
  if (deep) {
    console.warn(
      "we do not support use of deep flag on web.isInstance anymore"
    );
  }
  if (obj && equals) {
...
var type = (web.isType = function (obj, equals, deep) {
  ...
  //was it null?
  if (!obj) {
    //do not put this any higher because it will only work after you check with 'typeof'
  ...
  if (obj.concat === dummyArray.concat) {
    //or obj instanceof Array){fastest //or Array.isArray(obj) //or Object.getPrototypeOf(obj) == Array.prototype
    ...
    if (obj instanceof Array) {
      //Array.isArray(obj){ //we did a dirty fast check but now confirm
...
web.isEvent = function (obj) {
  //TODO use web instanceof for jquery and any other events
  return obj instanceof window.Event || obj instanceof $.Event;
};
web.toString = function (obj, override) {
  ...
  //handle xml
  if (
    web.instanceOf(obj, window.Document) &&
    !web.instanceOf(obj, window.HTMLDocument) &&
    !web.instanceOf(obj, window.SVGDocument)
  ) {
...
web.typeString = function (str) {
  ...
  if (firstChar == "." || firstChar == "/") {
    //relative path url put
  } else if (/^.{4,7}:\/\//.test(str)) {
    //absolute path uri
  }
};
...
web.isNode =
  typeof web.global.Node != "undefined"
    ? function (o) {
...
web.isElement =
  typeof web.global.HTMLElement != "undefined"
    ? function (o) {
...
web.contains = function (str, word, caseInsensitive) {
  ...
  if (caseInsensitive) {
    if (web.isString(word)) {
      ...
      if (!bank) {
        //Using reg-ex because http://jsperf.com/case-sensitive-regex-vs-case-insensitive-regex/3
...
var startsWith = function (str, prefix, caseInsensitive) {
  //note calling this externally helps prevent recursive array searching on prefix
...
var endsWith = function (str, suffix, caseInsensitive) {
  //note calling this externally helps prevent recursive array searching on prefix
  if (str && suffix) {
    if (caseInsensitive) {
      str = str.toLowerCase();
  ...
  if (!level) {
    web.isArray(obj) &&
      obj.every(function (o) {
        return web.isObject(o);
      });
  }
  return (
    web.isArray(obj) &&
    web.isObject(obj[0]) &&
    web.isObject(obj[web.toInt(obj.length / 2)]) &&
    web.isObject(obj[obj.length - 1])
  );
};
web.isArrayMatix = function (obj, level) {
  if (!level) {
    web.isArray(obj) &&
      obj.every(function (o) {
        return web.isArray(o);
      });
  }
  return (
    web.isArray(obj) &&
    web.isArray(obj[0]) &&
    web.isArray(obj[web.toInt(obj.length / 2)]) &&
    web.isArray(obj[obj.length - 1])
  );
};
...
web.isObject = function (obj, level) {
  if (!level) {
    return (
      isType(obj) == "Object"
    ); /*excludes array and null and regexp and HTMLelment etc*/ //Object.getPrototypeOf(obj) ===Object.prototype
  } else {
...
var properties = function (o, level) {
  ...
  for (p in o) {
    if (Object.prototype.hasOwnProperty.call(o, p)) {
      if (
        level &&
        Object.prototype.propertyIsEnumreable &&
        level != "properties"
      ) {
...
web.keys = function (obj, enumLevel, iterNonObject /*sort???*/) {
  ...
  if (iterNonObject) {
    ...
    if (!enumLevel || enumLevel == "keys") {
      //web.startsWith(enumLevel,'key')
      return Object.keys(obj);
    } else if (enumLevel == "properties") {
      //web.startsWith(enumLevel,'propert')
      return Object.getOwnPropertyNames(obj);
    } else if (enumLevel == "nonEnumerables") {
      //web.startsWith(enumLevel,'nonEnumerable')
...
web.hashSwap = function (data, fn) {
  //fn handles collisions
  // var ret = {};
  // for(var key in json){
  //   ret[json[key]] = key;
  // }
...
web.trimStart = function (str, word, instance, keepMatchChars) {
  // instance=-1=deep negitive numbers count from lastindexof
  if (!word) {
    //then trim start whitespace
    return str.replace(web.RegExp.leadingWhitespace, "");
  }

  var countType;
  if (web.isString(instance)) {
    //instance expected to be a number 5 (strict) or '5<' (loose count) or '5!' (n or no trim)
  ...
  if (type == "string") {
    ...
    while (instance != 0) {
      ...
      if (i < 0) {
        //was not found
        if (countType == "<") {
          //return last answer or '' if instances where more than found ins tring
          i = h;
          break;
        } else if (countType == "=") {
          return "";
...
web.trimRight = function (str, word, keep, deep) {
  //TODO make deep not boolean but rather number. so we can choose to cut at the 4th instance of a character etc
...
web.deepTrimRight = function (str, word, keep) {
  //todo possible rename to slash? maybe confusing though
...
web.queryString = function (url, variable, replace) {
  ...
  if (web.isValue(replace)) {
    ...
    //inspiration http://stackoverflow.com/questions/901115/how-can-i-get-query-string-values-in-javascript
...
var queryStringParser = function (value /*,assignment,delimiter */) {
  //setting assigment and delimiter not really good
  var q = {};
  value.replace(web.RegExp.queryStringParser, function ($0, $1, $2, $3) {
    if (q[$1]) {
      if (web.isArray(q[$1])) {
...
web.urlAddProtocol = function (url) {
  web.depricated(
    "web.urlAddProtocol will be depricated soon and will be integrated to web.url"
  );
  var protocol = window.location.protocol || "http:";
  if (web.startsWith(url, "//")) {
    url = protocol + url;
  } else if (
    !(web.startsWith(url, "http://") || web.startsWith(url, "https://"))
  ) {
    url = protocol + "//" + url;
  }
  return url;
...
web.toAbsoluteURL = function (url) {
  //TODO any callls to this web.toAbsoulteURL should use toAbsoluteURL if they are within web.js closure or use web.url if they are outside (aka in a project)
  web.depricated(
    "web.toAbsoluteURL will be depricated soon and will be integrated to web.url"
  );
...
var toAbsoluteURL = function (url) {
  ...
  if (!url) {
    return;
  } //url||location.href; i dont think i should do this just return undefined
...
web.url = function (url, cmd1, cmd2) {
  ...
  if (cmd1) {
    ...
    if (web.instanceOf(url, window.File) || web.instanceOf(url, window.Blob)) {
      if (web.startsWith(cmd1, "data", true)) {
        //todo figure out what this should be called. just take anything as true
  ...
  if (!(this instanceof web.url)) {
    return new web.url(url, cmd1, cmd2);
  }
  ...
  /*the browser does a simple trim left.
									...
									"#dfs"#lkjsdf=9"
									*/
  this.apply = function (part, data, replace) {
    ...
    if (part === "") {
      ...
      this["?"] = queryStringParser(
        web.deepTrimRight(web.trimLeft(this.url, "?"), "#")
      ); //NOTE this is how the browser does it!
    } else if (part == "#") {
      throw "not implmented";
    } else if (part == "#?") {
